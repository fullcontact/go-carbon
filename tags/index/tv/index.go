...
func (t *Tree) extract(q *d, i int) { // (r *TagValueInode) {
	t.ver++
...
func (t *Tree) First() (k string, v *TagValueInode) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k string) (v *TagValueInode, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k string, v *TagValueInode) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k string, v *TagValueInode) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k string, v *TagValueInode) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k string, v *TagValueInode) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k string, upd func(oldV *TagValueInode, exists bool) (newV *TagValueInode, write bool)) (oldV *TagValueInode, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV *TagValueInode
	if q == nil {
...
func (t *Tree) split(p *x, q *d, pi, i int, k string, v *TagValueInode) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k string, v *TagValueInode, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k string, v *TagValueInode, err error) {
	if err = e.err; err != nil {
		return
	}
