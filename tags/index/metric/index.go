...
func (t *Tree) extract(q *d, i int) { // (r *MetricInode) {
	t.ver++
...
func (t *Tree) First() (k string, v *MetricInode) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k string) (v *MetricInode, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k string, v *MetricInode) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k string, v *MetricInode) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k string, v *MetricInode) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k string, v *MetricInode) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k string, upd func(oldV *MetricInode, exists bool) (newV *MetricInode, write bool)) (oldV *MetricInode, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV *MetricInode
	if q == nil {
...
func (t *Tree) split(p *x, q *d, pi, i int, k string, v *MetricInode) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k string, v *MetricInode, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k string, v *MetricInode, err error) {
	if err = e.err; err != nil {
		return
	}
