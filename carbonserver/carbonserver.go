...
func (listener *CarbonserverListener) prepareData(format string, targets []string, fromTime, untilTime int32) (fetchResponse, error) {
	...
	for _, metric := range targets {
		...
		if strings.Contains(metric, ";") {
			metric = strings.Replace(metric, "_DOT_", ".", -1)
			fmt.Println("prepareData.path = ", []string{tags.FilePath("", metric, false)})
			res, err := listener.fetchDataPB(metric, []string{tags.FilePath("", metric, false)}, []bool{true}, fromTime, untilTime)
			if err != nil {
				atomic.AddUint64(&listener.metrics.RenderErrors, 1)
...
func (listener *CarbonserverListener) fetchDataPB(metric string, files []string, leafs []bool, fromTime, untilTime int32) (*pb.MultiFetchResponse, error) {
	...
	for i, metric := range files {
		...
		fmt.Printf("fetchDataPB.metric = %+v\n", metric)
...
func (listener *CarbonserverListener) statTagHandler(wr http.ResponseWriter, req *http.Request) {
	...
	if stat != nil {
		var resp pb.StatTagResponse
		resp.Tag = stat.Tag
		for _, val := range stat.Values {
			resp.Values = append(resp.Values, &pb.TagValue{Count: int32(val.Count), Value: val.Value})
		}
		switch format {
		case "json":
			contentType = "application/json"
...
func (listener *CarbonserverListener) listTagsHandler(wr http.ResponseWriter, req *http.Request) {
	...
	for _, tag := range tags {
		resp.Tags = append(resp.Tags, &pb.Tag{Tag: tag})
	}
	switch format {
	case "json":
		contentType = "application/json"
...
func (listener *CarbonserverListener) seriesByTagHandler(wr http.ResponseWriter, req *http.Request) {
	...
	response, _, err := listener.fetchWithCache(logger, format, paths, from, until)
